...
def add_column():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('ALTER TABLE properties ADD COLUMN diplay_image TEXT')
        #cursor.execute('DELETE FROM properties WHERE id = ?', (9,))
        #conn.commit()
        #conn.close()
...
def add_property():
    if 'logged_in' in session:
        if request.method == 'POST':
            ...
            diplay_image = request.files.getlist('main_image')
            
            
            print(diplay_image)
            if diplay_image and allowed_file(diplay_image[0].filename):
                    filename = secure_filename(diplay_image[0].filename)  
                    try:
                        diplay_image[0].save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                        image_main = filename 
                    except Exception as e:
            ...
            print(user_id, price, area, bedrooms,\
                            bathrooms, parking, year_built, nearby_places, \
                           contact, image_main ,images, rent_or_buy)
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(''' 
                INSERT INTO properties (user_id, price, area, bedrooms,\
                            bathrooms, parking, year_built, nearby_places, \
                           contact ,images, rent_or_buy,diplay_image)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (user_id, price, area, bedrooms, bathrooms, parking, year_built, nearby_places, contact, image_filenames_str, rent_or_buy,image_main))
            conn.commit()
            conn.close()
